...
[System.Serializable]
public class History_SS : MonoBehaviour {
	
	public Text ss_display;

	public static List<string> historySS = new List<string>();
	...
	public static void setNewAP() {
		string st = username + "  -  " + date_time + "  -  Score: " + scoreAchieved + "  -  Level: " + levelAchieved;
		historySS.Add (st);
		Login.current.addToSSRemove (st);
		Save ();
	}
	public static string printValues() {
		string textToReturn = "";

		foreach (string st in historySS) {
			textToReturn += st + "\n";
		}
	...
	public static void Save() {
		...
		FileStream file = File.Create (Application.persistentDataPath + "/ssHistory.dat");
	...
	public static void Load() {
		...
		if (File.Exists (Application.persistentDataPath + "/ssHistory.dat")) {
			//if it does, make a binary formatter to read the data, and open the file
			BinaryFormatter bf = new BinaryFormatter();
			FileStream file = File.Open (Application.persistentDataPath + "/ssHistory.dat", FileMode.Open);
