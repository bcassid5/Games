...
[System.Serializable]
public class History_MU : MonoBehaviour {

	public Text mu_display;

	public static List<string> historyMU = new List<string>();
	...
	public static void setNewAP() {
		...
		historyMU.Add (st);
		Login.current.addToMURemove (st);
		Save ();
	}
	public static string printValues() {
		string textToReturn = "";

		foreach (string st in historyMU) {
			textToReturn += st + "\n";
		}
	...
	public static void Save() {
		...
		FileStream file = File.Create (Application.persistentDataPath + "/muHistory.dat");
	...
	public static void Load() {
		...
		if (File.Exists (Application.persistentDataPath + "/muHistory.dat")) {
			//if it does, make a binary formatter to read the data, and open the file
			BinaryFormatter bf = new BinaryFormatter();
			FileStream file = File.Open (Application.persistentDataPath + "/muHistory.dat", FileMode.Open);
